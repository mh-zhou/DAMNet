...
# import time
# from model import MobileV2Net  # 确保你有这个模型的定义
...
# # 定义主函数
# def main():
#     model = MobileV2Net(class_num=4).to(device)  # 确保模型类别数量正确
#     model.load_state_dict(torch.load("experiencedata/ours/epoch200/newmodel_weights.pth", map_location=device))  # 加载模型权重
...
#     # 裁剪模型
#     parameters_to_prune = [(module, 'weight') for _, module in model.named_modules() if isinstance(module, torch.nn.Conv2d)]
...
#     validate_model(model, val_loader)  # 再次使用验证集验证
